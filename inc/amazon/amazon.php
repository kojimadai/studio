...
// 		add_action( 'media_buttons', array( $this, 'media_buttons' ), 20 );
		add_action( 'media_upload_amazon', array( $this, 'media_upload_amazon' ) );
		add_action( 'media_upload_amazon_keyword', array( $this, 'media_upload_amazon_keyword' ) );
		add_action( 'media_upload_amazon_id', array( $this, 'media_upload_amazon_id' ) );
	...
	function admin_print_styles() {
		...
		if ( version_compare( $wp_version, '3.8', '>=' ) ) {
			wp_enqueue_style( 'amazon-options', $this->url . '/inc/amazon/css/amazon-options.css', array(), self::VERSION );
		}
	}
	...
	function wp_enqueue_styles() {
		...
		if ( $this->settings['overrideThemeCss'] ) {
			wp_enqueue_style( 'amazon', $this->url . '/inc/amazon/css/amazon-force.css', array(), self::VERSION );
		} else {
			wp_enqueue_style( 'amazon', $this->url . '/inc/amazon/css/amazon.css', array(), self::VERSION );
		}
		if ( $this->settings['customCss'] ) {
			wp_enqueue_style( 'amazon-custom', get_stylesheet_directory_uri() . '/amazon.css' );
		}
	}

	function wp_enqueue_scripts() {
		wp_register_script( 'jquery-tmpl', $this->url . '/inc/amazon/jquery.tmpl.min.js', array( 'jquery' ), '1.0.0pre', true );

		$depends = array( 'jquery-tmpl' );
		if ( $this->settings['displayCustomerReview'] ) {
			$depends[] = 'thickbox';
		}
		wp_register_script( 'amazon', $this->url . '/inc/amazon/js/amazon.js', $depends, self::VERSION, true );
		if ( $this->settings['customJs'] ) {
			wp_register_script( 'amazon-custom', get_stylesheet_directory_uri() . '/amazon.js', array( 'amazon' ), self::VERSION, true );
		}
	}
	...
	function wp_enqueue_scripts_for_footer() {
		...
		$this->enqueue_amazon_scripts( $items, $country_codes );
	}

	function enqueue_amazon_scripts( $items = array(), $country_codes = array() ) {
		...
		wp_localize_script( 'amazon', 'amazonVars', $amazonVars );

		wp_enqueue_script( 'amazon' );
		if ( $this->settings['customJs'] ) {
			wp_enqueue_script( 'amazon-custom' );
		}
	}
	...
	function init_settings() {
		...
		$this->setting_fields = array(
			...
			'displayCustomerReview'     => array(
				...
				'type'        => 'checkbox',
				'section'     => 'appearance',
				'description' => __( "amazon will display customer review by using WordPress's Thickbox.", $this->text_domain ),
			),
			'supportDisabledJavascript' => array(
				'label'       => __( 'Display official widget when disabled javascript in web browser', $this->text_domain ),
				'type'        => 'checkbox',
				'section'     => 'appearance',
				'description' => __( 'If set to true, amazon will output html by using <code>&lt;script type=&quot;text/javascript&quot;&gt;document.write(&quot;{$indicator_html}&quot;)&lt;/script&gt;&lt;noscript&gt;{$link_html}&lt;/noscript&gt;</code>.', $this->text_domain ),
			),
			'useAnimation'              => array(
			...
			'overrideThemeCss'          => array(
				...
				'type'        => 'checkbox',
				'section'     => 'appearance',
				'description' => __( 'If set to true, amazon will override the style of the theme by using <code>!important</code> declaration.', $this->text_domain ),
			),
			'useTrackEvent'             => array(
				'label'       => __( 'Click Tracking by using Google Analytics', $this->text_domain ),
				'type'        => 'checkbox',
				'section'     => 'analytics',
				'description' => __( 'If set to true, amazon will call <code>_gaq.push(["_trackEvent", "amazon", "Click", "ASIN TITLE"])</code> or <code>ga("send", "event", "amazon", "Click", "ASIN TITLE")</code>.', $this->text_domain ),
			),
			'customCss'                 => array(
				'label'       => __( 'Use Custom Css', $this->text_domain ),
				'type'        => 'checkbox',
				'section'     => 'customize',
				'description' => '(' . $template_url . '/amazon.css)',
			),
			'customJs'                  => array(
				'label'       => __( 'Use Custom Javascript', $this->text_domain ),
				'type'        => 'checkbox',
				'section'     => 'customize',
				'description' => '(' . $template_url . '/amazon.js)',
			),
		);
...
EOF;
		...
		$item = (array_key_exists( $asin, $this->display_items[ $country_code ] ))
			...
			: $this->display_items[ $country_code ][ $asin ] =  get_site_transient("amazon_{$country_code}_{$asin}");
		$url  = '#';
		if ( is_array( $item ) && array_key_exists( 'DetailPageURL', $item ) ) {
			$url = $item['DetailPageURL'];
		}
		$indicator_html = <<<EOF
<div data-role="amazon" data-asin="{$asin}" data-locale="{$country_code}" data-tmpl="${tmpl}" data-img-size="${imgsize}" class="asin_{$asin}_{$country_code}_${tmpl} amazon_item"><div class="amazon_indicator"><span class="amazon_indicator_img"></span><a class="amazon_indicator_title" href="{$url}">{$title}</a><span class="amazon_indicator_footer"></span></div></div>
EOF;
...
EOF;
	...
	function get_item( $country_code, $asin ) {
		if ( $ai = get_site_transient( "amazon_{$country_code}_{$asin}" ) ) {
			return $ai;
		}
		...
		while ( count( $item_ids ) ) {
			...
			if ( $results && $results['success'] ) {
				foreach ( $results['items'] as $item ) {
					...
					set_site_transient("amazon_{$country_code}_{$item['ASIN']}", $item, self::CACHE_LIFETIME);
				}
			}
		}
		return $items;
	...
	function plugin_row_meta( $links, $file ) {
		if ( $file == $this->plugin_rel_file ) {
			...
			array_push(
				$links,
				sprintf( '<a href="https://github.com/makotokw/wp-amazon/releases" target="_blank">%s</a>', __( 'Old Releases', $this->text_domain ) )
			);
		}
		return $links;
	}
...
/*
	function media_buttons() {
		...
		?>
		<a href="<?php echo esc_attr( $iframe_src . '&amp;TB_iframe=true' ); ?>" id="add_amazon" class="button thickbox" title="<?php echo esc_attr( $label ); ?>"><img src="<?php echo esc_attr( $this->url . '/inc/amazon/images/amazon-icon.png' ); ?>" alt="<?php echo esc_attr( $label ); ?>"/></a>
	<?php
	}
*/
	...
	function media_upload_init() {
		...
		wp_enqueue_style( 'amazon-media-upload', $this->url . '/inc/amazon/css/media-upload-type-amazon.css', array( 'amazon' ), self::VERSION );

		$this->enqueue_amazon_scripts();
	}

	function media_upload_amazon() {
		$this->media_upload_init();
		wp_iframe( 'media_upload_type_amazon' );
	}

	function media_upload_amazon_keyword() {
		$this->media_upload_init();
		wp_iframe( 'media_upload_type_amazon_keyword' );
	}

	function media_upload_amazon_id() {
		$this->media_upload_init();
		wp_iframe( 'media_upload_type_amazon_id' );
	}
	...
	function ajax_amazon_search() {
		...
		$amazon->init();
		if ( isset($itemId) ) {
			$result = $amazon->itemlookup( $countryCode, $itemId );
			die(json_encode( $result ));
		} else {
			$result = $amazon->itemsearch( $countryCode, $searchIndex, $keywords, $itemPage );
			die(json_encode( $result ));
		}
	}
	...
	function amazon_get( $countryCode, $options ) {
		...
		$accessKeyId     = 'AKIAJCPUVVNWOOY7IEPA';
		$secretAccessKey = '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';
		$associateTag = get_theme_mod('amazon_id','monetizeconsu-22');
		...
		if ( WP_DEBUG ) {
			if ( ! $xml ) {
				error_log( 'amazon: cannot parse xml: ' . $body );
			}
		}
...
function media_upload_type_amazon() {
	include dirname( __FILE__ ) . '/media-upload-type-amazon.php';
}

function media_upload_type_amazon_keyword() {
	include dirname( __FILE__ ) . '/media-upload-type-amazon.php';
}

function media_upload_type_amazon_id() {
	include dirname( __FILE__ ) . '/media-upload-type-amazon.php';
}
...
add_action( 'init', 'amazon_init' );
if ( function_exists( 'register_uninstall_hook' ) ) {
	register_uninstall_hook( __FILE__, 'amazon_uninstall' );
}
